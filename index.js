...
function indekseGoreFenomen(dizi, indekstekiFenomen) {
  
  let x = indekstekiFenomen + ". indekste bulunan fenomen: " + dizi[indekstekiFenomen].profile;
...
function fenomenSil(dizi, indeksSayisi) {

  let yeniDizi = [...dizi];

  yeniDizi.shift(indeksSayisi);
...
function fenomenEkle(dizi, sayi, profil, takipci, gonderi, platform) {

  let yeniDizi = [...dizi];

  const yeniHesap = {};

  yeniHesap.number = sayi;
  yeniHesap.profile = profil;
  yeniHesap.followers = takipci;
  yeniHesap.posts = gonderi;
  yeniHesap.platform = platform;

  yeniDizi.push(yeniHesap);  // yeniDizi.splice(4,0, yeniHesap) -> komutuyla instagram platform grubunun altında 6. sıraya ekleyebiliriz böylelikle göze daha hoş gözükür.
...
function fenomenGonderimSayisi(dizi, fenomenAdlari){

  let gonderimSayisi;

  for (let i = 0; i < dizi.length; i++) {
    
    if (fenomenAdlari === dizi[i].profile) {

      gonderimSayisi = dizi[i].posts;
...
function platformaGoreCokGonderiYapanFenomen(dizi, platform){
  ...
  for (let i = 0; i < dizi.length; i++) {
    ...
    if (dizi[i].platform === platform) {
      ...
      if (dizi[i].posts !== "NA" && dizi[i].posts > enCokGonderi) {

        enCokGonderi = dizi[i].posts;
        feno = dizi[i].profile;
      }
